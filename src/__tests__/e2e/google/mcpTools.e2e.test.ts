...
describe("Google MCP Tool Basic Validation E2E", () => {
  ...
  beforeAll(async () => {
    ...
    const { GoogleGeminiV1Provider } = await import(
      "../../../providers/google-gemini-v1/googleGeminiV1Provider"
    );
  ...
  describe("MCP Tool Execution", () => {
    test("should execute MCP tool through Google model", async () => {
      // Clear any previous tool call history
      mcpServer.clearToolCallHistory();

      // Create test input for echo tool
      const testInput: string = "Hello MCP from Google";
