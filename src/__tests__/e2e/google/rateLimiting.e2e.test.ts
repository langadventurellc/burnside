...
describe("Google Rate Limiting E2E", () => {
  ...
  beforeAll(() => {
    // Load Google test configuration and validate environment
  ...
  describe("Configuration Validation", () => {
    test("should not throttle when rate limiting is disabled", async () => {
      ...
      const unthrottledClient = createRateLimitedTestClient("google", {
        enabled: false,
        maxRps: 2,
        scope: "provider",
      });
  ...
  describe("Scope Testing", () => {
    test("should isolate rate limits between different provider scopes", async () => {
      ...
      const client1 = createRateLimitedTestClient("google", {
        enabled: true,
        maxRps: 2,
        scope: "provider",
      });

      const client2 = createRateLimitedTestClient("google", {
        enabled: true,
        maxRps: 2,
        scope: "provider",
      });
    ...
    test("should isolate rate limits between different model scopes", async () => {
      ...
      const modelScopedClient = createRateLimitedTestClient("google", {
        enabled: true,
        maxRps: 2,
